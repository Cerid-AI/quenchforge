...
func (g *Gateway) handleOllamaEmbeddings() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		// Pre-emptive back-off if AUTO_BACKOFF is on and the routed INSTANCE
		// is shedding (critical error rate). Same contract as the
		// OpenAI-shaped path.
		...
		serve := func() {
			...
			defer resp.Body.Close()
			g.latency.Record(track, time.Since(started), resp.StatusCode >= 500)
