...
|---|---|---|
| `amd-gpu-integration` | `ci.yml` | `go test -tags=amd_gpu ./tests/integration/...` on every push to main and every PR. Reproduces the patch's "coherent output" assertion against the live Vega II. |
